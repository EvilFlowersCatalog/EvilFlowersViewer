...
const Share = (params: IShareParams) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (isInappropriate) { // if it's inappropriate (1,4, or 1,5-), return
      return;
    }
    params.setLink(''); // reset link
    params.setShareQRVisibility(true); // open ShareQRCode component

    if (shareFunction) { // function exists
  ...
  const handleInput = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (pattern.test(eventInput) && !pattern2.test(eventInput)) {
      setInput(eventInput)
      if (eventInput[eventInput.length - 1] === ',' || eventInput[eventInput.length - 1] === '-') { // if string ends with , or - do not share
   ...
   */
  const handleExpare = (expaire: { name: string, value: number, active: boolean }) => {
    // update expaire
  ...
  return (
    <div className={'relative px-4 text-center gap-2'}>
      <form className='w-full' onSubmit={handleSubmit}>
        ...
        <div className='mt-3 text-left'>
          <span className='text-sm text-gray-800 dark:text-gray-200'>{t('shareExpaire')}</span>
        ...
        {
          <div className={'absolute top-0 end-4'}>
            <Tooltip placement='left' title={t('shareSearchInfoToolTip')}>
              <BiInfoCircle className={'w-[18px] h-[18px] mt-2 text-blue-600 dark:text-blue-300'} />
            </Tooltip>
          </div>
        }
      </form>
