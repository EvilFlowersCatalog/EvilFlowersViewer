...
<template>
  ...
  <div
    ...
    :class="docStore.sidebarState === SIDEBAR_STATE.NULL ? 'w-0' : 'w-[207px]'"
  >
    <!-- Inner: always 207px wide so content doesn't squish -->
    <div class="flex flex-col w-[207px] h-full bg-white dark:bg-[#1e1e1e] rounded-r-[5px] shadow-[2px_0px_8px_rgba(0,0,0,0.12)]">

      <!-- Header -->
      <div class="flex items-center justify-between px-[7px] py-3 border-b border-gray-200 dark:border-white/10 shrink-0">
        <h2
          v-if="docStore.sidebarState !== SIDEBAR_STATE.NULL"
          class="text-[13px] font-medium uppercase text-[#333] dark:text-gray-200 tracking-[0.1px]"
        >
      ...
      <!-- Content (scrollable) -->
      <div class="flex-1 overflow-auto px-[7px] py-4">
