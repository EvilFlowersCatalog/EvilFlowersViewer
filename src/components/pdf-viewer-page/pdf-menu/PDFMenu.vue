...
<template>
  <div
    class="flex items-center h-12 px-4 shrink-0 z-30 gap-0"
    :class="isDark ? 'border-b border-white/5' : 'border-b border-black/5'"
    :style="{ backgroundColor: props.topbarBg ?? '#2C2C2C' }"
  >

    <!-- ═══ FAR LEFT: Home + TOC ═══ -->
    <div class="flex items-center gap-3">
      <ToolTip v-if="viewerStore.homeFunction" position="bottom" :text="$t('home')">
      ...
      <ToolTip v-if="docStore.toc.length > 0" position="bottom" :text="$t('table-of-content')">
    ...
    <!-- Divider -->
    <div class="w-px h-5 mx-4 shrink-0" :class="isDark ? 'bg-white/15' : 'bg-black/10'"></div>

    <!-- ═══ CENTER-LEFT: Search + Edit + Citation ═══ -->
    <div class="flex items-center gap-3">
      <!-- Search -->
      <ToolTip position="bottom" :text="$t('search')">
      ...
      <!-- Edit -->
      <ToolTip v-if="viewerStore.config.edit && viewerStore.editPackage" position="bottom" :text="docStore.edit ? $t('edit-disable') : $t('edit-enable')">
      ...
      <!-- Citation -->
      <ToolTip v-if="viewerStore.citation" position="bottom" :text="$t('citation')">
    ...
    <!-- Flex spacer pushes center block to true center -->
    <div class="flex-1"></div>

    <!-- ═══ CENTER CONTROLS: Zoom + Fullscreen + Page Nav ═══ -->
    <div class="flex items-center gap-1.5 shrink-0">
      <!-- Zoom out -->
      <ToolTip position="bottom" :text="$t('zoom-out')">
        <button
          @click="docStore.zoomOut"
          class="w-7 h-7 flex items-center justify-center rounded text-lg font-bold transition-colors"
          :class="docStore.scale <= 0.25
            ? (isDark ? 'text-white/20' : 'text-black/20') + ' cursor-not-allowed'
            : isDark ? 'text-white/70 hover:text-white hover:bg-white/10' : 'text-gray-500 hover:text-gray-900 hover:bg-black/5'"
        >−</button>
      </ToolTip>

      <!-- Zoom % -->
      <span class="text-xs w-10 text-center select-none font-medium tabular-nums" :class="isDark ? 'text-white/80' : 'text-gray-700'">{{ zoomPercent }}</span>

      <!-- Zoom in -->
      <ToolTip position="bottom" :text="$t('zoom-in')">
        <button
          @click="docStore.zoomIn"
          class="w-7 h-7 flex items-center justify-center rounded text-lg font-bold transition-colors"
          :class="docStore.scale >= 3
            ? (isDark ? 'text-white/20' : 'text-black/20') + ' cursor-not-allowed'
            : isDark ? 'text-white/70 hover:text-white hover:bg-white/10' : 'text-gray-500 hover:text-gray-900 hover:bg-black/5'"
        >+</button>
      </ToolTip>
      ...
      <!-- Fullscreen -->
      <ToolTip position="bottom" :text="!docStore.isFullscreenMode ? $t('fullscreen-enable') : $t('fullscreen-disable')">
        <button @click="toggleFullScreen" class="w-7 h-7 flex items-center justify-center rounded transition-colors" :class="isDark ? 'text-white/70 hover:text-white hover:bg-white/10' : 'text-gray-500 hover:text-gray-900 hover:bg-black/5'">
      ...
      <!-- Divider -->
      <div class="w-px h-4 mx-1 shrink-0" :class="isDark ? 'bg-white/15' : 'bg-black/10'"></div>

      <!-- Prev page -->
      <button
        @click="docStore.previousPage"
        class="w-6 h-7 flex items-center justify-center rounded transition-colors"
        :class="docStore.activePage === 1
          ? (isDark ? 'text-white/20' : 'text-black/20') + ' cursor-not-allowed'
          : isDark ? 'text-white/70 hover:text-white hover:bg-white/10' : 'text-gray-500 hover:text-gray-900 hover:bg-black/5'"
      >
      ...
      <div class="flex items-center gap-1">
        <input
          name="search-page"
          class="w-10 h-7 rounded text-center text-xs font-medium outline-none focus:border-secondary transition-colors tabular-nums"
          :class="isDark
      ...
      <button
        ...
        class="w-6 h-7 flex items-center justify-center rounded transition-colors"
        :class="docStore.activePage === docStore.totalPages
          ? (isDark ? 'text-white/20' : 'text-black/20') + ' cursor-not-allowed'
          : isDark ? 'text-white/70 hover:text-white hover:bg-white/10' : 'text-gray-500 hover:text-gray-900 hover:bg-black/5'"
      >
        <svg class="size-3" viewBox="0 0 8 12" fill="none">
    ...
    <!-- Flex spacer pushes center-right group away from center -->
    <div class="flex-1"></div>

    <!-- ═══ CENTER-RIGHT: Share + Info + Print + Download ═══ -->
    <div class="flex items-center gap-3">
      <!-- Share -->
      <ToolTip v-if="viewerStore.shareFunction && viewerStore.config.share" position="bottom" :text="$t('share')">
      ...
      <!-- Info -->
      <ToolTip position="bottom" :text="$t('info')">
      ...
      <!-- Print -->
      <ToolTip v-if="viewerStore.config.print && viewerStore.printFunction" position="bottom" :text="$t('print')">
      ...
      <!-- Download -->
      <ToolTip v-if="viewerStore.config.download" position="bottom" :text="$t('download')">
    ...
    <!-- ═══ FAR RIGHT: Dark mode + Close ═══ -->
    <div class="flex items-center gap-3">
      <!-- Dark mode toggle -->
      <ToolTip position="bottom" :text="isDark ? 'Light mode' : 'Dark mode'">
        <button @click="$emit('toggleDark')" :class="btnClass(false)">
          <!-- Sun (in dark mode → click for light) -->
      ...
      <!-- Close -->
      <ToolTip v-if="viewerStore.closeFunction" position="bottom" :text="$t('close')">
        <button @click="viewerStore.closeFunction" class="w-8 h-8 flex items-center justify-center rounded transition-colors hover:text-red-400" :class="isDark ? 'text-white/60 hover:bg-white/10' : 'text-gray-400 hover:bg-black/5'">
          <svg class="icon" viewBox="0 0 20 20" fill="none">
